  ...
  webServer: (() => {
    ...
    return {
      command: useBuiltServer
        ? // dist/server/wrangler.json is generated by @cloudflare/vite-plugin
          // with `no_bundle: true` + `main: index.js`. Pointing wrangler at it
          // runs the already-built worker instead of re-bundling
          // src/server.ts (TanStack Start's virtual `#tanstack-*-entry`
          // imports aren't resolvable outside Vite).
