...
- A LITTLE SPOKEN DIALOGUE IS WELCOME — USED SPARINGLY. When a person is on screen and the moment genuinely earns it, let them say one short, natural line; the pipeline performs it as lip-synced audio, and a spoken beat brings a scene alive. Keep it to roughly a line or two across the WHOLE film — not every scene, and never a back-and-forth conversation — with most beats still carrying the story visually. Each line must be short enough to speak inside a ~5-second clip (a handful of words). Write it as something a character SAYS in the action (e.g. she grins and says, "Told you."), never as on-screen subtitles or a "VO:"/voiceover block.
