...
- **Local dev:** `bun dev:bunny` runs the export service on the host under the bun runtime (no Docker — `node-av`/FFmpeg works under bun); it's also included in `bun dev:all`. To make the running app actually call it, set `VIDEO_EXPORT_DEV_URL=http://localhost:8080` in `.env.local` — the workflow then POSTs there instead of the (production-only) container binding. Unset in prod, so prod uses the container. The container uses **bun** as its package manager (`bun.lock`, `trustedDependencies: ["node-av"]`) but **Node** as the image runtime. Container details, contract, and Docker build/smoke-test: `containers/video-export/README.md`.
